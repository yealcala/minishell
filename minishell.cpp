 ...
 *  - Versión interactiva: Ejecutar ./a.out
 *  - Versión fichero de comandos: Ejecutar ./a.out fichero
 */
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) { // Interactivo.
        ...
        ifstream entrada;
        entrada.open(argv[1]);
        if (entrada.fail()) { // Si no existe el archivo introducido...
            cerr << "No puede abrirse el archivo " << argv[1] << endl;
            exit(2);
        } else {
            char **trozos;
            entrada.getline(cmd, INPUT_MAX_SIZE, '\n'); 
            while (! entrada.eof() && strcmp(cmd, EXIT) != 0 ) {
                int num = strsplit(cmd, ";", trozos);
                for(int i = 0; i < num; i++)
                    ejecutar(strtrim(*(trozos+i)));
                entrada.getline(cmd, INPUT_MAX_SIZE, '\n'); 
            }
            free(trozos);
            entrada.close();
        }
    } else { // Si se introducen 2 o más argumentos...
